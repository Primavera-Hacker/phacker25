...
import { ImprovedNoise, MeshSurfaceSampler } from "three/examples/jsm/Addons.js";
import { abs,floor, atan, atan2, mx_noise_float,cos, float, length, max, sqrt, step, uv, vec2, vec3, vec4, smoothstep, sin, time, mix, positionLocal, positionWorld, mod, round, clamp, distance, mx_noise_vec3 } from "three/tsl";
...
export default class PrimaveraHacker
{
    ...
    init()
    {
        ...
        let   dist_center = sin(_uv.sub(vec2(0.5)).length().add(time.mul(-0.05)).mul(30.0)).mul(0.5).add(0.5);
        const radius = float(0.2);
        const thickness = float(0.05);
        let linea = smoothstep(radius.add(thickness), radius, dist_center).mul(smoothstep(radius.sub(thickness), radius, dist_center));
        linea = step(0.05,linea);   


        let   color_floor = vec3(mx_noise_float(_pix_uv.mul(10.0).add(vec3(0.0,0.0,time.mul(0.3))),1.0));
        color_floor = mix(vec3(0.0),vec3(0.05,0.1,.0),color_floor);

        floor_material.colorNode = mix(color_floor,vec3(10.0,0.0,.0),linea);


        const floor_instance = new THREE.Mesh(new THREE.PlaneGeometry(floor_size,floor_size,12,12).rotateX(-Math.PI*.5),floor_material);
       ...
       //     this.glitch.set_hide(1.0);
        ...
        const y_offset = round(float(instanceIndex.add(time.mul(2))).div(char_count));
        ...
        const textura_ascii = texture(this.experience.resources.items.ascii,uv_ascii)
        const positionAttribute = new THREE.InstancedBufferAttribute( new Float32Array( positions ), 3 );
        const material = new THREE.SpriteNodeMaterial( { sizeAttenuation: true, alphaTest: 0.1 } );
        
        
        material.positionNode = instancedBufferAttribute( positionAttribute );
        material.scaleNode = distance(material.positionNode,vec3(0.0)).mul(0.03).mul(hash(instanceIndex).add(0.4).mul(2.0))
